...
const fullDateLabel = (value: string) => new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'UTC' }).format(new Date(`${value}T00:00:00Z`))
...
function AllocationDonut({ items, total, highlightedId, onHighlight }: { items: Distribution[]; total: number; highlightedId: string | null; onHighlight: (id: string | null) => void }) {
  let cursor = 0
  const slices = items.map(item => {
    const start = Math.min(cursor, 100)
    const size = Math.max(0, Math.min(numeric(item.percentage), 100 - start))
    cursor += Math.max(0, numeric(item.percentage))
